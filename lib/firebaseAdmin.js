...
function resolveCredential() {
  ...
  const absolute = path.resolve(process.cwd(), creds);
  const safeRoot = path.resolve(process.cwd());

  if (!absolute.startsWith(safeRoot + path.sep) && absolute !== safeRoot) {
    console.error('GOOGLE_APPLICATION_CREDENTIALS path traversal detected. Path must be within the project root.');
    return admin.credential.applicationDefault();
  }

  if (path.extname(absolute) !== '.json') {
    console.error('GOOGLE_APPLICATION_CREDENTIALS must point to a .json file.');
    return admin.credential.applicationDefault();
  }
